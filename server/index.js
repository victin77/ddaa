...
async function initDb() {
  ...
  for (const c of consultants) {
    ...
    if (!u) {
      const hash = await bcrypt.hash(CONSULTANT_DEFAULT_PASSWORD, 10);
      await db.run(
...
async function pendingByConsultant({ today }) {
  const sql = `
    SELECT
      s.consultant_name as name,
      SUM(CASE WHEN i.paid_date IS NULL AND i.status!='paid' AND i.due_date < ? THEN 1 ELSE 0 END) as overdue,
      SUM(CASE WHEN i.paid_date IS NULL AND i.status!='paid' AND i.due_date >= ? THEN 1 ELSE 0 END) as pending
    FROM installments i
    JOIN sales s ON s.id = i.sale_id
    GROUP BY s.consultant_name
    ORDER BY (pending + overdue) DESC, s.consultant_name ASC
  `;
  const rows = await db.all(sql, [today, today]);
  return (rows || []).map(r => ({
    name: r.name,
    pending: Number(r.pending || 0),
    overdue: Number(r.overdue || 0),
    total: Number(r.pending || 0) + Number(r.overdue || 0)
  }));
}

async function nextUnpaidInstallment({ consultant_id, today }) {
  const where = ["i.paid_date IS NULL", "i.status!='paid'"];
  const params = [];
  if (consultant_id != null) { where.push("s.consultant_id=?"); params.push(consultant_id); }
  // order: upcoming first (>= today), then overdue (oldest)
  params.push(today);
  const sql = `
    SELECT
      i.sale_id,
    ...
    ORDER BY
      CASE WHEN i.due_date >= ? THEN 0 ELSE 1 END ASC,
      i.due_date ASC,
      i.number ASC
    LIMIT 1
  `;
  ...
  const status = (row.due_date < today) ? 'overdue' : 'pending';
  return {
    sale_id: row.sale_id,
...
app.get('/api/summary', auth(), async (req, res) => {
  ...
  const [pending_by_consultant, next_installment] = await Promise.all([
    pendingByConsultant({ today }),
    nextUnpaidInstallment({ consultant_id: cid, today })
    nextUnpaidInstallment({ consultant_id: cid, today })
...
// =========================
// Installments breakdown (counts only)
// =========================
app.get('/api/installments/pending-by-consultant', auth(), async (req, res) => {
  const today = ymd(new Date());
  const rows = await pendingByConsultant({ today });
  res.json({ as_of: today, rows });
});
// =========================
// Export Excel (.xlsx)
// =========================
