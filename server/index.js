...
import {
  ADMIN_PASSWORD,
  ADMIN_USER,
  CANCELLATION_PHASE_DAYS,
  DB_INIT_RETRIES,
  DB_INIT_RETRY_DELAY_MS,
  JWT_SECRET,
  PORT,
  RESET_CONSULTANT_PASSWORDS,
  RESET_SALES_DATA,
  assertAdminSeedIsSafe,
...
for (const method of ['get', 'post', 'put', 'delete']) {
  ...
  app[method] = (routePath, ...handlers) =>
    original(
      routePath,
      ...handlers.map((handler) => (typeof handler === 'function' && handler.length < 3 ? asyncHandler(handler) : handler))
    );
}
/*
...
async function initDb() {
  ...
  for (const c of consultants) {
    ...
    if (!existingLogin) {
      ...
      const rawPassword = resolveConsultantPassword(username, c.id);
      ...
      if (RESET_CONSULTANT_PASSWORDS) {
        const rawPassword = resolveConsultantPassword(normalizeUsername(existingLogin.username), c.id);
        const hash = await bcrypt.hash(rawPassword, 10);
...
async function startServer() {
  ...
  for (let attempt = 1; attempt <= DB_INIT_RETRIES; attempt += 1) {
    try {
      ...
      lastError = error;
      console.error(
        `[startup] database init failed (attempt ${attempt}/${DB_INIT_RETRIES}):`,
