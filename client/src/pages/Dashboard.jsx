...
import { InfoBox, Modal, Pill, SectionTitle, SelectInput, Shell, TabButton, TextInput } from '../components/dashboard/ui.jsx';
import { ConsultantsView, SalesView, SettingsView } from '../components/dashboard/views.jsx';
import {
  Moon,
  Sun,
  LogOut,
...
export default function Dashboard({ user, onLogout }) {
  ...
  const downloadExcel = async () => {
    ...
    try {
      const { blob, filename } = await api.exportXlsx(scope);
  ...
  const saveInstallments = async () => {
    ...
    try {
      ...
      await api.updateInstallments(details.id, payloadInstallments);
        ...
        {loading ? (
          ...
          <>
            ...
            {tab === 'consultants' && isAdmin && (
              <ConsultantsView consultants={consultants} onReload={load} UsersIcon={Users} />
            )}

            {tab === 'settings' && isAdmin && (
              <SettingsView consultants={consultants} onReload={load} KeyRoundIcon={KeyRound} />
            )}
          </>
        )}
      </main>
