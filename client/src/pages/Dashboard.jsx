  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60]">
      <motion.div
        ...
        className={`rounded-2xl shadow-2xl border px-4 py-3 backdrop-blur-xl ${
          toast.type === 'success'
      ...
      >
        <div className="flex items-start gap-3">
          <div className="w-2 h-2 rounded-full mt-2 bg-current opacity-80" />
          <div className="text-sm">
            <div className="font-semibold">{toast.title || 'Tudo certo'}</div>
...
export default function Dashboard({ user, onLogout }) {
  ...
  const chartDonut = useMemo(() => ({
    ...
    options: { responsive: true, plugins: { legend: { position: 'bottom', labels: { color: darkMode ? '#e2e8f0' : '#0f172a' } } } }
  }), [kpis]);
  ...
  const saveSale = async (payload) => {
    try {
      if (editing) {
        ...
        setEditing(null);
        setToast({ type: 'success', title: 'Atualizado', message: 'Venda atualizada com sucesso ✅' });
      } else {
        await api.createSale(payload);
        setToast({ type: 'success', title: 'Parabéns!', message: 'Venda registrada! 🎉' });
      }
      await load();
      setTimeout(() => setToast(null), 3500);
    } catch (e) {
      setToast({ type: 'error', title: 'Erro', message: 'Não foi possível salvar a venda.' });
      setTimeout(() => setToast(null), 3500);
      throw e;
    }
  };
  ...
  const saveQuotas = async () => {
    ...
    try {
      ...
      await load();
      setDetails(updated);
      setQuotasMsg('✅ Cotas salvas.');
    } catch (e) {
      setQuotasMsg('❌ Não foi possível salvar as cotas.');
    } finally {
              ...
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow-xl">
                    <SectionTitle icon={BarChart3} title="Ranking de consultores" subtitle="Top 3 do time" />
                    <div className="mt-4">
                      {isAdmin ? (
                        <>
                        ...
                        <div className="space-y-3">
                          {ranking.slice(0, 3).map((r, idx) => (
                            <div key={r.name} className="flex items-center justify-between rounded-2xl border border-slate-200/60 dark:border-white/10 bg-white/60 dark:bg-white/5 px-4 py-3">
                              <div className="text-slate-800 dark:text-slate-100 font-semibold">
                                {idx === 0 ? '🥇' : idx === 1 ? '🥈' : '🥉'} #{idx+1} {r.name}
                              </div>
                              <div className="text-xs text-slate-500 dark:text-slate-300">posição</div>
                            </div>
                          ))}
                          <div className="text-xs text-slate-500 dark:text-slate-300">
                            *Valores de vendas/comissões ficam visíveis apenas para o ADM.
                          </div>
                        </div>
                      )}
                    </div></div>
                ...
                {summary?.pending_by_consultant && summary.pending_by_consultant.length > 0 && (
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
                    <div className="lg:col-span-2 rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow-xl">
                      <SectionTitle icon={Users} title="Parcelas pendentes por consultor" subtitle="Quantidade (pendente + atrasado)" />
                      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          {summary.pending_by_consultant.slice(0, 10).map((r) => (
                            <div key={r.name} className="flex items-center justify-between text-sm">
                              ...
                              <div className="font-semibold text-slate-900 dark:text-white">
                                ...
                                <span className="ml-2 text-xs text-slate-500 dark:text-slate-300">
                                  ({r.pending} pend · {r.overdue} atr)
                                </span>
                              </div>
                            </div>
                          ))}
                        </div>
                        <div className="rounded-2xl border border-slate-200/60 dark:border-white/10 bg-white/60 dark:bg-white/5 p-4">
                          <div className="text-xs text-slate-500 dark:text-slate-300 mb-2">Top consultores (por quantidade)</div>
                          <Bar
                            data={{
                              labels: summary.pending_by_consultant.slice(0, 6).map(x => x.name),
                              datasets: [{
                                label: 'Qtd',
                                data: summary.pending_by_consultant.slice(0, 6).map(x => x.total),
                            ...
                            options={{
                              ...
                              plugins: { legend: { display: false } },
                              scales: {
                                x: { ticks: { color: darkMode ? '#cbd5e1' : '#334155' }, grid: { color: darkMode ? 'rgba(148,163,184,0.15)' : 'rgba(100,116,139,0.12)' } },
                                y: { ticks: { color: darkMode ? '#cbd5e1' : '#334155' }, grid: { color: darkMode ? 'rgba(148,163,184,0.15)' : 'rgba(100,116,139,0.12)' } }
                              }
                            }}
                          />
                        </div>
                    ...
                    <div className="rounded-3xl border border-slate-200/60 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-xl p-6 shadow-xl">
                      <SectionTitle icon={FileText} title={isAdmin ? "Próxima parcela do time" : "Sua próxima parcela"} subtitle="Mais próxima a vencer (ou a mais atrasada)" />
                      {summary.next_installment ? (
                        <div className="mt-4 space-y-3">
                          <div className="rounded-2xl border border-slate-200/60 dark:border-white/10 bg-white/60 dark:bg-white/5 p-4">
                            <div className="text-sm font-semibold text-slate-900 dark:text-white">
                              Parcela #{summary.next_installment.number}
                              <span className="ml-2 text-xs text-slate-500 dark:text-slate-300">
                                ({statusLabel(summary.next_installment.status)})
                              </span>
                            </div>
                            <div className="mt-2 text-sm text-slate-700 dark:text-slate-200">
                              Venc.: <span className="font-semibold">{fmtDate(summary.next_installment.due_date)}</span>
                            </div>
                            {!isAdmin && (
                              <div className="mt-2 text-xs text-slate-500 dark:text-slate-300">
                                Cliente: {summary.next_installment.client_name} • {summary.next_installment.product}
                              </div>
                            )}
                            {isAdmin && (
                              <div className="mt-2 text-xs text-slate-500 dark:text-slate-300">
                                {summary.next_installment.consultant_name} • {summary.next_installment.client_name}
                              </div>
                            )}
                          </div>
                          <div className="text-xs text-slate-500 dark:text-slate-300">
                            Dica: marque como paga na aba Parcelas quando confirmar o recebimento.
                          </div>
                        </div>
                      ) : (
